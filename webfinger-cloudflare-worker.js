...
// Test cases:
// https://seifried.org/.well-known/webfinger?resource=acct:ted@tedt.org
// https://seifried.org/.well-known/webfinger?resource=acct:ted@tedt.org2
...
async function handleRequest(request) {
  ...
  if (resourceData) {
    ...
    const gravatar = '{"rel": "http://webfinger.net/rel/avatar","href": "https://secure.gravatar.com/avatar/' + MD5(email) + '?s=512&d=mp&r=g"}';

    const jsonData = '{"subject":"acct:' + email + '","aliases":["https://' + hostname + '/@' + username + '","https://' + hostname + '/users/' + username + '"],"links":[{"rel":"http://webfinger.net/rel/profile-page","type":"text/html","href":"https://' + hostname + '/@' + username + '"},{"rel":"self","type":"application/activity+json","href":"https://' + hostname + '/users/' + username + '"},{"rel":"http://ostatus.org/schema/1.0/subscribe","template":"https://' + hostname + '/authorize_interaction?uri={uri}"},'+gravatar+']}';
...
addEventListener('fetch', async event => {
  ...
  // Throw a passthrough exception to process the rest of the site.
  event.passThroughOnException();
  throw new Error('Do the rest of the site');
});
